...
export function PlanLoadDialog({ open, onClose, onSelect }: PlanLoadDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div
        className="w-full max-w-md rounded-xl border border-[var(--store-line)] bg-[var(--store-bg)] shadow-xl"
      ...
      >
        ...
        <div className="max-h-72 overflow-y-auto p-2">
          {loading && <p className="text-[11px] font-mono store-text-muted px-2 py-3">Loading plans…</p>}
          {error && <p className="text-[11px] font-mono text-red px-2 py-3">{error}</p>}
          {!loading && !error && plans.length === 0 && (
            <p className="text-[11px] font-mono store-text-muted px-2 py-3">No saved plans — use Save first.</p>
          )}
          {plans.map((p) => (
            <button
              key={p.id}
              type="button"
              className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-[var(--store-surface-2)] border border-transparent hover:border-[var(--store-line)] transition-colors"
