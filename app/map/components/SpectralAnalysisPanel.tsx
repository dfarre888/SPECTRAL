      ...
      <SheetContent side="right" className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader>
          <div className="flex items-center justify-between gap-2">
            <SheetTitle className="flex items-center gap-2">
              <Radio className="w-4 h-4 text-[var(--wb-blue)]" />
        ...
        {!hasPlaced ? (
          <p className="mt-6 text-sm store-text-body">
            ...
            <span className="block mt-2 text-[11px] store-text-muted font-mono">
          ...
          <div className="mt-4 space-y-6 pb-8">
            <section className="rounded-xl border border-[rgba(41,151,255,0.5)] bg-[rgba(41,151,255,0.14)] p-3 space-y-2">
              ...
              {analysis.copilot.reasoning && analysis.copilot.reasoning.length > 0 && (
                <ul className="text-[11px] store-text-muted space-y-1 list-disc pl-4">
            ...
            <section className="space-y-3">
              ...
              {placedEmissions.length === 0 ? (
                <p className="text-[11px] store-text-muted">No spectrum emissions resolved for placed assets.</p>
              ) : (
                <>
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-[11px] store-text-muted mt-1 leading-snug">{pair.spectrum.detail}</p>
      </div>
          ...
          <ul className="space-y-1">
            {pair.bandOverlaps.map((o) => (
              <li
                ...
                className="text-[11px] font-mono text-t-secondary flex gap-1"
              >
        ...
        <div>
          ...
          <ul className="space-y-0.5">
            {pair.uncoveredGaps.map((g) => (
              <li key={g.id} className="text-[11px] font-mono text-t-muted">
                ○ {g.label}
              </li>
            ))}
          </ul>
      ...
      <div>
        ...
        <ul className="space-y-0.5">
          {pair.uasSurvivalTactics.map((t) => (
            <li key={t} className="text-[11px] text-t-secondary leading-snug">
              → {t}
            </li>
          ))}
        </ul>
    ...
    <div className="rounded-xl store-panel-inner p-2.5">
      ...
      {profile.bands.length === 0 ? (
        ...
        <ul className="space-y-0.5 max-h-28 overflow-y-auto">
          {profile.bands.map((b) => (
            <li key={`${b.label}-${b.range}`} className="text-[11px] font-mono flex justify-between gap-2">
        ...
        <div className="mt-2 pt-2 border-t border-[var(--store-line)]">
          ...
          <ul className="space-y-0.5">
            {profile.redundancies.map((r) => (
              <li key={r} className="text-[11px] text-green leading-snug">
                + {r}
              </li>
            ))}
          </ul>
