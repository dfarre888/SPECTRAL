    ...
    <div className="w-[320px] rounded-xl border border-[var(--store-line)] bg-[var(--store-surface)] shadow-2xl flex flex-col max-h-[calc(100vh-120px)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 text-xs">
        <label className="block space-y-1">
          <span className="text-[11px] uppercase store-text-muted">System</span>
          ...
          {envelope && (
            <p className="text-[11px] font-mono store-text-muted pt-0.5">
        ...
        <div className="space-y-1">
          <span className="text-[11px] uppercase store-text-muted">Target</span>
          <div className="flex flex-wrap gap-1">
            {TARGET_CATEGORIES.map((cat) => (
              <button key={cat} type="button" onClick={() => setTarget(cat)} className={cn('rounded px-2 py-1 text-[11px] font-mono border', target === cat ? 'bg-[#F97316] border-[#F97316] text-white' : 'border-[var(--store-line)] store-text-muted')}>
                {cat.replace(/_/g, ' ')}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="space-y-1">
            <span className="text-[11px] store-text-muted">Slant range (m)</span>
          ...
          <label className="space-y-1">
            <span className="text-[11px] store-text-muted">Target alt (m)</span>
            <div className="relative">
              {altBand && (
                <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-1.5 rounded-full bg-black/40 pointer-events-none">
                  <div
        ...
        <div className="space-y-1">
          <span className="text-[11px] store-text-muted">Salvo</span>
        ...
        <div className="space-y-1">
          <span className="text-[11px] store-text-muted">ECM</span>
          <div className="flex flex-wrap gap-1">{ECM_LEVELS.map((level) => (
            <button key={level} type="button" onClick={() => setEcmLevel(level)} className={cn('rounded px-2 py-1 text-[11px] font-mono border', ecmLevel===level ? 'bg-[#F97316] border-[#F97316] text-white' : 'border-[var(--store-line)] store-text-muted')}>{level.replace(/_/g,' ')}</button>
          ))}</div>
        </div>
        {result && (
          <div className="rounded-lg border border-[var(--store-line)] bg-[var(--store-surface)] p-3 space-y-3">
            {check && (
              <p className="text-[11px] store-text-body leading-snug">{check.statement}</p>
            )}
            <div className={cn('text-center text-[11px] font-semibold uppercase py-1 rounded', result.in_envelope ? 'bg-emerald-950/80 text-emerald-400' : 'bg-[#7F1D1D] text-[#EF4444]')}>
              ...
              <div className="space-y-1.5">
                {check.failures.map((f) => (
                  <div key={f.axis} className="rounded border border-red-500/25 bg-red-500/5 px-2 py-1.5">
                    <p className="text-[11px] text-red-200 leading-snug">{f.message}</p>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="mt-1 text-[11px] font-mono text-cyan hover:underline"
                    >
            ...
            {result.in_envelope && (
              <>
                ...
                <div className="text-[11px] font-mono store-text-muted space-y-1 border-t border-[var(--store-line)] pt-2">
                ...
                {result.engagement_notes.length > 0 && (
                  <ul className="text-[11px] store-text-body list-disc pl-3 space-y-0.5">{result.engagement_notes.map((n) => <li key={n}>{n}</li>)}</ul>
                )}
                {result.recommended_response && <p className="text-[11px] store-text-body">{result.recommended_response}</p>}
              </>
            )}
          </div>
        )}
...
function PkRow({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-[11px] store-text-muted"><span>{label}</span><span className="font-mono text-xl" style={{ color }}>{value.toFixed(2)}</span></div>
