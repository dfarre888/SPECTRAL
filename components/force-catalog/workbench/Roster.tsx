        ...
        <div className="ml-auto flex gap-0.5" role="group" aria-label="Sort roster">
          {(['name', 'nation', 'tier'] as const).map((k) => (
            <button key={k} type="button" aria-pressed={sort === k} onClick={() => setSort(k)}
              className={`text-[11px] font-mono px-1.5 py-1 rounded capitalize transition-colors duration-150 ${sort === k ? 'store-text-body bg-[var(--store-surface-2)]' : 'store-text-muted hover:store-text-body'}`}>
              {k}
            </button>
          ))}
        </div>
