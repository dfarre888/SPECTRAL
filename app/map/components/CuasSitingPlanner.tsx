    ...
    >
      <div className="p-3 space-y-3 text-[11px] store-text-body">
        ...
        <div className="rounded-lg border border-[var(--store-line)] px-2 py-2" style={{ borderColor: verdictColor }}>
          <p className="text-[11px] store-text-muted">Coverage verdict</p>
          <p className="text-sm font-semibold uppercase" style={{ color: verdictColor, ...mono }}>{analysis.verdict}</p>
          <p className="text-[11px] mt-1" style={mono}>
            {analysis.covered_count}/{analysis.total_threats} threats · {analysis.coverage_pct}%
          </p>
        </div>
        {analysis.gaps.length > 0 && (
          <div>
            <p className="text-[11px] text-[#F97316] mb-1">Coverage gaps</p>
            <ul className="space-y-1 max-h-32 overflow-y-auto">
              {analysis.gaps.map((g) => (
                <li key={g.uas_id} className="text-[11px] store-panel-inner rounded px-2 py-1">
        ...
        {analysis.siting_recommendations.map((rec, i) => (
          <div key={i} className="store-panel-inner rounded-lg px-2 py-2 border-l-2 border-[#F97316]">
            <p className="text-[11px] text-[#F97316]">Siting recommendation</p>
            <p className="text-[11px] text-white" style={mono}>
              {rec.lat.toFixed(5)}, {rec.lon.toFixed(5)}
            </p>
            <p className="text-[11px] store-text-muted mt-1">{rec.rationale}</p>
            <p className="text-[11px] mt-1" style={mono}>+{rec.expected_additional_coverage} threat(s)</p>
          </div>
        ))}
        <p className="text-[11px] store-text-muted" style={mono}>{analysis.performance_ref}</p>
      </div>
    </div>
  )
}
